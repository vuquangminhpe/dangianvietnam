...
export const createTheater = async (
  theaterData: TheaterCreateRequest
): Promise<TheaterCreateResponse> => {
  try {
    const staffApi = createStaffRequest();
...
export const updateTheater = async (
  theaterId: string,
  theaterData: TheaterUpdateRequest
): Promise<TheaterUpdateResponse> => {
  try {
    const staffApi = createStaffRequest();
    const response = await staffApi.put(
      `/staff/theater/${theaterId}`,
      theaterData
    );
    return response.data;
  } catch (error) {
...
export const checkMovieTitleExists = async (
  title: string
): Promise<boolean> => {
  try {
...
export const getMyMovieById = async (
  movieId: string
...
export const getMyMovieRatings = async (
  ...
  try {
    ...
    const response = await staffApi.get(`/staff/movies/${movieId}/ratings`, {
      params,
    });
    return response.data;
  } catch (error) {
...
export const getMyMovieFeedbacks = async (
  ...
  try {
    ...
    const response = await staffApi.get(`/staff/movies/${movieId}/feedbacks`, {
      params,
    });
    return response.data;
  } catch (error) {
...
export const isValidMovieStatus = (
  status: string
): status is "coming_soon" | "now_showing" | "ended" => {
...
export const validateMovieData = (
  movieData: MovieCreateRequest | MovieUpdateRequest
): string[] => {
  const errors: string[] = [];
  ...
  if (
    !movieData.duration ||
    movieData.duration < 1 ||
    movieData.duration > 500
  ) {
